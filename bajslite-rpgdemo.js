javascript:(async function(){
 ...
 const APP_VERSION = 'lite 6.14-rpg-loader';
 const DEBUG = true;
 const debugBuffer = [];
...
margin-left: 2px !important;
 ...
 bOfflineTimers.set(k, timer);
     ...
     const RPG_PLUGIN_URL='https://raw.githubusercontent.com/jeffvanss1/Bajsaslite/refs/heads/main/rpg-plugin.js';
     let rpgLoading=null;
     ...
     rpgBtn.onclick=async()=>{
       if(window.BajSASRPG?.loaded)return window.BajSASRPG.open();
       if(rpgLoading)return rpgLoading;
       const original=rpgBtn.textContent;rpgBtn.disabled=true;rpgBtn.textContent='☢ Loading…';dbg('rpg:loading',{url:RPG_PLUGIN_URL});
       rpgLoading=(async()=>{try{const response=await fetch(RPG_PLUGIN_URL+'?t='+Date.now(),{cache:'no-store'});if(!response.ok)throw new Error('HTTP '+response.status);const code=await response.text();try{(0,eval)(code)}catch(evalError){dbg('rpg:eval-fallback',{error:String(evalError)});await new Promise((resolve,reject)=>{const blob=new Blob([code],{type:'text/javascript'}),src=URL.createObjectURL(blob),script=document.createElement('script');script.src=src;script.onload=()=>{URL.revokeObjectURL(src);script.remove();resolve()};script.onerror=()=>{URL.revokeObjectURL(src);script.remove();reject(new Error('Plugin script blocked'))};document.head.appendChild(script)})}if(!window.BajSASRPG?.loaded)throw new Error('Plugin did not initialize');window.BajSASRPG.open();rpgBtn.textContent='☢ RPG';dbg('rpg:loaded',{version:window.BajSASRPG.version})}catch(error){console.error('[BajSAS RPG] Load failed',error);dbg('rpg:load-error',{error:String(error)});rpgBtn.textContent='☢ Retry RPG'}finally{rpgBtn.disabled=false;rpgLoading=null}})();
       return rpgLoading;
     };btnRow.appendChild(rpgBtn);
