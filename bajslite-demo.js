...
margin-left: 2px !important;
 ...
 const MEDIA_PROVIDER_LOCAL_PRIORITY_KEY='bajsas_main2_local_provider_priority',MEDIA_PROVIDER_PRIORITY_WAIT_MS=2500,mediaProviderSources=['vixsrc','cinesrc','vidfast'];
 ...
 let mediaPreferredProvider=(()=>{try{const saved=String(localStorage.getItem(MEDIA_PROVIDER_LOCAL_PRIORITY_KEY)||'').toLowerCase();return mediaProviderSources.includes(saved)?saved:''}catch{return''}})();
 ...
 const mediaSetPreferredProvider=source=>{mediaPreferredProvider=mediaProviderSources.includes(source)?source:'';try{if(mediaPreferredProvider)localStorage.setItem(MEDIA_PROVIDER_LOCAL_PRIORITY_KEY,mediaPreferredProvider);else localStorage.removeItem(MEDIA_PROVIDER_LOCAL_PRIORITY_KEY)}catch{}mediaRefreshProviderMenu();dbg('media:local-provider-priority',{source:mediaPreferredProvider||'none'})};
