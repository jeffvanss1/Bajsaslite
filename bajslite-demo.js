...
margin-left: 2px !important;
 ...
 const mediaPanel=document.createElement('div');mediaPanel.className='bajsas-media';mediaPanel.innerHTML='<iframe allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen></iframe><div class="bajsas-media-lock">Admin synchronized playback</div><div class="bajsas-media-fetch-warning" data-media-fetch-warning>Taking longer than expected? Close Chrome DevTools/Console, switch away, then reopen MAIN 2.</div><div class="bajsas-media-bar"><input placeholder="IMDb link/ID, TMDB ID, TV ID season episode, or VidFast URL"><button class="custom-stream-btn" data-media-add>Request</button><button class="custom-stream-btn" data-media-vote>Vote Skip</button><span class="bajsas-media-queue" data-media-status>No media</span><button class="custom-stream-btn" data-media-close>✕</button></div>';wrapper.appendChild(mediaPanel);
 const mediaButton=document.createElement('button');mediaButton.className='custom-stream-btn';mediaButton.textContent='▶ MAIN 2';mediaButton.title='Media Share watch party';mediaButton.dataset.mediaChannel='1';const mainButton=[...btnRow.querySelectorAll('.custom-stream-btn[data-stream-selector="1"]')].find(button=>(button.dataset.streamName||'').toLowerCase()==='main');if(mainButton)mainButton.insertAdjacentElement('afterend',mediaButton);else btnRow.appendChild(mediaButton);
 let mediaFrame=mediaPanel.querySelector('iframe');const mediaInput=mediaPanel.querySelector('input'),mediaStatus=mediaPanel.querySelector('[data-media-status]'),mediaFetchWarning=mediaPanel.querySelector('[data-media-fetch-warning]');let mediaFetchTimer=null;const createMediaFrame=()=>{const frame=document.createElement('iframe');frame.allow='autoplay; fullscreen; encrypted-media; picture-in-picture';frame.setAttribute('allowfullscreen','');mediaPanel.prepend(frame);return frame};const mediaQueueList=document.createElement('div');mediaQueueList.className='bajsas-media-queue-list';document.body.appendChild(mediaQueueList);let mediaCurrentId='',mediaCurrentServer='',mediaAttemptId='',mediaAttemptStartedAt=0,mediaReadyKey='',mediaRevision=0,mediaSuppressUntil=0,mediaClockOffset=0,mediaLastState=null,mediaHoverCard=null,mediaLastObservedTime=-1,mediaLastAdvancedAt=Date.now(),mediaBufferReportedKey='';
 const renderMediaQueue=state=>{mediaQueueList.replaceChildren();const queue=state?.queue||[],ownNextId=queue.find(item=>item.requestedBy===bGetUser())?.id||'';if(queue.length&&!state?.confirmationOpen){const note=document.createElement('div');note.className='bajsas-media-queue-meta';note.style.padding='7px';note.textContent='Confirmation opens during the final minute of the current movie.';mediaQueueList.appendChild(note)}if(!queue.length){const empty=document.createElement('div');empty.className='bajsas-presence-empty';empty.textContent='Queue is empty';mediaQueueList.appendChild(empty);return}queue.forEach((item,index)=>{const row=document.createElement('div');row.className='bajsas-media-queue-item';const number=document.createElement('span');number.className='bajsas-media-queue-index';number.textContent=String(index+1);const info=document.createElement('div');const title=document.createElement('div');title.className='bajsas-media-queue-title';title.textContent=item.title+(item.year?' ('+item.year+')':'');const meta=document.createElement('div');meta.className='bajsas-media-queue-meta';meta.textContent='Requested by '+item.requestedBy+' • '+(item.confirmedBy?.length||0)+' confirmed';info.append(title,meta);const action=document.createElement('span');if(state?.confirmationOpen&&item.requestedBy===bGetUser()&&item.id===ownNextId){const confirm=document.createElement('button');confirm.className='custom-stream-btn';confirm.textContent=item.confirmedBy?.includes(bGetUser())?'✓':'Confirm';confirm.disabled=item.confirmedBy?.includes(bGetUser());confirm.onclick=event=>{event.stopPropagation();sendMedia({type:'media_confirm',itemId:item.id})};action.appendChild(confirm)}else{action.className='bajsas-media-queue-meta';action.textContent=item.confirmedBy?.length?'✓':''}row.append(number,info,action);mediaQueueList.appendChild(row)})};
 ...
 const mediaCommand=payload=>mediaFrame.contentWindow?.postMessage(payload,'*');
 const applyMediaPlayback=cur=>{const pb=cur?.playback;if(!pb)return;const expected=Math.max(0,pb.position+(pb.playing?(mediaNow()-pb.updatedAt)/1000:0));mediaSuppressUntil=Date.now()+1000;mediaCommand({command:'seek',time:expected});mediaCommand({command:pb.playing?'play':'pause'});mediaRevision=pb.revision||0};
 const enforceMediaPlayback=data=>{const cur=mediaLastState?.current,pb=cur?.playback;if(!pb||Date.now()<mediaSuppressUntil)return;const expected=Math.max(0,pb.position+(pb.playing?(mediaNow()-pb.updatedAt)/1000:0)),actual=Math.max(0,Number(data.currentTime)||0),wrongState=Boolean(data.playing)!==Boolean(pb.playing),drift=Math.abs(actual-expected);if(wrongState||drift>1.25){mediaSuppressUntil=Date.now()+900;if(drift>.5)mediaCommand({command:'seek',time:expected});mediaCommand({command:pb.playing?'play':'pause'});dbg('media:authoritative-correction',{expected:+expected.toFixed(2),actual:+actual.toFixed(2),drift:+drift.toFixed(2),playing:pb.playing})}};
 const renderMedia=state=>{mediaLastState=state;if(mediaQueueList.classList.contains('show'))renderMediaQueue(state);if(Number.isFinite(Number(state.serverNow))&&Math.abs((Number(state.serverNow)-Date.now())-mediaClockOffset)>2000)mediaClockOffset=Number(state.serverNow)-Date.now();const cur=state.current,main2Visible=mediaPanel.classList.contains('show')&&String(bActiveStream||'').toLowerCase().replace(/\s+/g,'')==='main2';mediaStatus.textContent=cur?`${cur.title}${cur.year?' ('+cur.year+')':''} • by ${cur.requestedBy} • ${state.votes}/${state.required} votes • ${state.queue.length} queued`:'Queue empty';if(!main2Visible){if(mediaCurrentId)destroyMediaPlayer();return}if(cur&&(cur.id!==mediaCurrentId||String(cur.server||'')!==mediaCurrentServer)){mediaCurrentId=cur.id;mediaCurrentServer=String(cur.server||'vRapid');mediaAttemptId=String(cur.sourceAttemptId||'');mediaAttemptStartedAt=Date.now();mediaLastObservedTime=-1;mediaLastAdvancedAt=Date.now();mediaBufferReportedKey='';const pb=cur.playback||{playing:true,position:0,updatedAt:cur.startedAt};const start=Math.max(0,Math.floor(pb.position+(pb.playing?(mediaNow()-pb.updatedAt)/1000:0)));mediaFrame.src=`${WORKER_BASE_URL}/media-player?type=${encodeURIComponent(cur.type)}&id=${encodeURIComponent(cur.providerId)}&season=${Number(cur.season||1)}&episode=${Number(cur.episode||1)}&start=${start}&server=${encodeURIComponent(mediaCurrentServer)}`;clearTimeout(mediaFetchTimer);mediaFetchWarning.classList.remove('show');mediaFetchTimer=setTimeout(()=>{mediaFetchWarning.classList.add('show');sendMedia({type:'media_source_failed',mediaId:mediaCurrentId,server:mediaCurrentServer,attemptId:mediaAttemptId,reason:'fetch_timeout'})},12000);mediaFrame.onload=()=>setTimeout(()=>applyMediaPlayback(cur),700)}else if(cur&&(cur.playback?.revision||0)>mediaRevision)applyMediaPlayback(cur);if(!cur){mediaCurrentId='';mediaCurrentServer='';mediaAttemptId='';mediaAttemptStartedAt=0;mediaReadyKey='';mediaLastObservedTime=-1;mediaLastAdvancedAt=Date.now();mediaBufferReportedKey='';mediaRevision=0;mediaFrame.src=''}};
 const destroyMediaPlayer=()=>{clearTimeout(mediaFetchTimer);mediaFetchWarning.classList.remove('show');mediaPanel.classList.remove('show');mediaQueueList.classList.remove('show');const oldFrame=mediaFrame;try{oldFrame.contentWindow?.postMessage({command:'pause'},'*');oldFrame.contentWindow?.postMessage({command:'mute',muted:true},'*')}catch{}oldFrame.onload=null;try{oldFrame.src='about:blank'}catch{}oldFrame.remove();mediaFrame=createMediaFrame();mediaCurrentId='';mediaCurrentServer='';mediaAttemptId='';mediaAttemptStartedAt=0;mediaReadyKey='';mediaLastObservedTime=-1;mediaLastAdvancedAt=Date.now();mediaBufferReportedKey='';mediaRevision=0;mediaSuppressUntil=0;dbg('media:player-hard-destroyed')};
 let mediaLastProgressSent=0;const VIDFAST_ORIGINS=new Set(['https://vidfast.pro','https://vidfast.in','https://vidfast.io','https://vidfast.me','https://vidfast.net','https://vidfast.pm','https://vidfast.xyz','https://vidfast.vc','https://vidfast.bz']);
 window.addEventListener('message',event=>{if(!VIDFAST_ORIGINS.has(event.origin)||event.source!==mediaFrame.contentWindow)return;const payload=event.data;if(payload?.type==='MEDIA_DATA'){try{localStorage.setItem('bajsas_vidfast_progress',JSON.stringify(payload.data))}catch{}return}if(payload?.type!=='PLAYER_EVENT')return;const data=payload.data;if(!data)return;if(['timeupdate','playerstatus'].includes(data.event)){const observed=Number(data.currentTime)||0;if(data.playing!==false){if(observed>mediaLastObservedTime+.2){mediaLastObservedTime=observed;mediaLastAdvancedAt=Date.now();mediaBufferReportedKey=''}else if(Date.now()-mediaLastAdvancedAt>10000){const key=mediaCurrentId+'|'+mediaCurrentServer;if(mediaBufferReportedKey!==key){mediaBufferReportedKey=key;sendMedia({type:'media_source_failed',mediaId:mediaCurrentId,server:mediaCurrentServer,attemptId:mediaAttemptId,reason:'buffering'})}}}else{mediaLastObservedTime=observed;mediaLastAdvancedAt=Date.now()}}if(data.event==='play'||(data.event==='timeupdate'&&Number(data.currentTime)>0)){clearTimeout(mediaFetchTimer);mediaFetchWarning.classList.remove('show');const readyKey=mediaCurrentId+'|'+mediaCurrentServer;if(mediaReadyKey!==readyKey){mediaReadyKey=readyKey;sendMedia({type:'media_source_ready',mediaId:mediaCurrentId,server:mediaCurrentServer,attemptId:mediaAttemptId,startupMs:Date.now()-mediaAttemptStartedAt})}}if(data.event==='ended'){sendMedia({type:'media_ended',mediaId:mediaCurrentId,currentTime:Number(data.currentTime)||0,duration:Number(data.duration)||0});return}enforceMediaPlayback(data);if(Date.now()<mediaSuppressUntil)return;if(['timeupdate','playerstatus'].includes(data.event)&&Date.now()-mediaLastProgressSent>8000){mediaLastProgressSent=Date.now();sendMedia({type:'media_progress',time:Number(data.currentTime)||0,duration:Number(data.duration)||0,playing:data.playing!==false})}});
 setInterval(()=>{if(mediaPanel.classList.contains('show')&&mediaCurrentId)mediaCommand({command:'getStatus'})},1000);
 mediaButton.onclick=()=>{destroyMediaPlayer();bGameUiEnabled=false;bGameModal.classList.remove('show');v.muted=true;i.src='';i.style.display='none';bActiveStream='MAIN 2';bLastProcessedCh='MAIN 2';bApplyLocalWatch('MAIN 2');bPing('watch:MAIN 2');mediaPanel.classList.add('show');sendMedia({type:'media_get'});setActive(mediaButton)};mediaPanel.querySelector('[data-media-close]').onclick=()=>nativeBtn.click();mediaPanel.querySelector('[data-media-add]').onclick=()=>{if(mediaInput.value.trim()){sendMedia({type:'media_request',query:mediaInput.value.trim()});mediaInput.value=''}};mediaPanel.querySelector('[data-media-vote]').onclick=()=>sendMedia({type:'media_vote_skip'});
