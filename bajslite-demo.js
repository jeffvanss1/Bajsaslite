...
margin-left: 2px !important;
 ...
 const destroyMediaPlayer=()=>{mediaPanel.classList.remove('show');mediaQueueList.classList.remove('show');try{mediaCommand({command:'pause'});mediaCommand({command:'mute',muted:true})}catch{}mediaFrame.onload=null;mediaFrame.removeAttribute('src');mediaFrame.src='about:blank';mediaCurrentId='';mediaRevision=0;mediaSuppressUntil=0;dbg('media:player-destroyed')};
 ...
 bOfflineTimers.set(k, timer);
     ...
     const gameBtn=document.createElement('button');gameBtn.className='custom-stream-btn';gameBtn.textContent='🎮 Checkers';gameBtn.title='Play multiplayer checkers';gameBtn.onclick=()=>{destroyMediaPlayer();bOpenGame()};btnRow.appendChild(gameBtn);
