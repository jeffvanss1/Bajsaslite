...
margin-left: 2px !important;
 ...
 window.addEventListener('message',event=>{if(!VIDFAST_ORIGINS.has(event.origin)||event.source!==mediaFrame.contentWindow)return;const payload=event.data;if(payload?.type==='MEDIA_DATA'){try{localStorage.setItem('bajsas_vidfast_progress',JSON.stringify(payload.data))}catch{}return}if(payload?.type!=='PLAYER_EVENT')return;const data=payload.data;if(!data)return;if(data.event==='play'||(data.event==='timeupdate'&&Number(data.currentTime)>0)){clearTimeout(mediaFetchTimer);mediaFetchWarning.classList.remove('show')}if(data.event==='ended'){sendMedia({type:'media_ended',mediaId:mediaCurrentId,currentTime:Number(data.currentTime)||0,duration:Number(data.duration)||0});return}enforceMediaPlayback(data);if(Date.now()<mediaSuppressUntil)return;if(['timeupdate','playerstatus'].includes(data.event)&&Date.now()-mediaLastProgressSent>8000){mediaLastProgressSent=Date.now();sendMedia({type:'media_progress',time:Number(data.currentTime)||0,duration:Number(data.duration)||0,playing:data.playing!==false})}});
