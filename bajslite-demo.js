javascript:(async function(){
 ...
 const APP_VERSION = 'lite 5.5-lobby-fix-demo';
 const DEBUG = true;
 const debugBuffer = [];
...
margin-left: 2px !important;
 ...
 .bajsas-game-modal{display:none;position:absolute;right:14px;bottom:86px;z-index:2147483646;pointer-events:auto;width:min(360px,42vw)}.bajsas-game-modal.show{display:block;animation:bajGameIn .2s ease-out}.bajsas-game-modal.lobby-mode .bajsas-game-window{background:linear-gradient(155deg,#44108a,#6d16bb);border-color:#a36de0;color:#fff}.bajsas-game-modal.lobby-mode .bajsas-game-head{border-bottom:1px solid rgba(255,255,255,.2);padding-bottom:7px}.bajsas-game-modal.lobby-mode [data-room-name],.bajsas-game-modal.lobby-mode [data-room-time]{background:#fff!important;color:#2d1248!important;border:0!important;border-radius:7px!important}.bajsas-game-window{width:100%;background:linear-gradient(#514537,#2c261f);color:#f6e6bd;border:2px solid #88765c;border-radius:7px;padding:9px;box-shadow:0 18px 55px #000}.bajsas-game-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:7px}.bajsas-game-board{display:grid;grid-template-columns:repeat(8,1fr);aspect-ratio:1;border:4px ridge #9a815d;box-shadow:inset 0 0 18px #000}.bajsas-game-cell{display:grid;place-items:center;border:0;padding:0}.bajsas-game-cell.light{background:#c9b58d}.bajsas-game-cell.dark{background:#72543d}.bajsas-piece{width:72%;height:72%;border-radius:50%;box-shadow:inset 0 0 0 3px rgba(255,255,255,.18),0 3px 5px #000}.bajsas-piece.red{background:#d9382f}.bajsas-piece.black{background:#151515}.bajsas-piece.king:after{content:'★';display:grid;place-items:center;height:100%;color:#ffd54f;font-size:18px}.bajsas-game-cell.selected{outline:4px solid #69f0ae;outline-offset:-4px}.bajsas-game-controls{display:flex;gap:6px;align-items:center;margin-bottom:7px;flex-wrap:wrap}.bajsas-game-controls select{background:#211d18;color:#fff;border:1px solid #8b7759;padding:5px}.bajsas-lobby{background:linear-gradient(160deg,#3d0876,#6414a8);border:1px solid #8f52d1;padding:8px;border-radius:7px;max-height:310px;overflow:auto}.bajsas-room-card{background:#fff;color:#241238;border-radius:9px;padding:8px;margin-bottom:7px;box-shadow:0 3px 0 rgba(0,0,0,.18)}.bajsas-room-top{display:flex;justify-content:space-between;gap:8px;font-weight:800}.bajsas-room-meta{font-size:10px;color:#725d86;margin:4px 0 7px}.bajsas-room-actions{display:flex;gap:5px;justify-content:flex-end}.bajsas-room-actions button{border:0;border-radius:99px;padding:4px 11px;font-weight:800;cursor:pointer}.bajsas-room-join{background:#ffc928}.bajsas-room-watch{background:#d8c4ef;color:#482277}.bajsas-lobby-empty{text-align:center;padding:30px 8px;color:#e8d8ff}.bajsas-game-score{display:grid;grid-template-columns:1fr auto 1fr;gap:7px;align-items:center;margin:7px 0;text-align:center}.bajsas-game-player{background:rgba(0,0,0,.25);padding:5px;border:1px solid #76644c}.bajsas-game-clock{font:bold 16px monospace;color:#fff}.bajsas-game-actions{display:flex;justify-content:center;gap:6px;margin-top:7px}@keyframes bajGameIn{from{opacity:0;transform:translate3d(16px,10px,0) scale(.96)}to{opacity:1;transform:none}}@media(max-width:800px){.bajsas-game-modal{width:min(330px,90vw);right:8px;bottom:72px}}
 `;
 ...
 bOfflineTimers.set(k, timer);
     ...
     function bShowLobby(){bGameModal.classList.add('lobby-mode');bGamePlay.style.display='none';bRoomList.style.display='block';bGameBack.style.display='none';bGameStatus.textContent='Public Lobby';bGameSend({type:'checkers_lobby_list'});}
     ...
     function bRenderLobby(){bRoomList.replaceChildren();if(!bLobbyRooms.length){const empty=document.createElement('div');empty.className='bajsas-lobby-empty';empty.innerHTML='No public rooms yet.<br>Create one to start playing.';bRoomList.appendChild(empty);return;}for(const r of bLobbyRooms){const card=document.createElement('div');card.className='bajsas-room-card';const top=document.createElement('div');top.className='bajsas-room-top';const name=document.createElement('span');name.textContent=r.roomName;const cost=document.createElement('span');cost.textContent=(r.timeControlMin||5)+' min';top.append(name,cost);const meta=document.createElement('div');meta.className='bajsas-room-meta';meta.textContent=`Host: ${r.host}${r.opponent?' • vs '+r.opponent:''} • ${r.status} • ${r.spectators} watching`;const actions=document.createElement('div');actions.className='bajsas-room-actions';if(r.status==='open'&&r.host!==bGetUser()){const join=document.createElement('button');join.className='bajsas-room-join';join.textContent='Join';join.onclick=()=>bGameSend({type:'checkers_join_room',gameId:r.gameId});actions.appendChild(join)}const watch=document.createElement('button');watch.className='bajsas-room-watch';watch.textContent='Watch';watch.onclick=()=>bGameSend({type:'checkers_spectate',gameId:r.gameId});actions.appendChild(watch);card.append(top,meta,actions);bRoomList.appendChild(card)}}
