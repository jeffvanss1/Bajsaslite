...
margin-left: 2px !important;
 ...
 .bajsas-media{display:none;position:absolute;inset:0;z-index:2147483644;background:#0e0e10;pointer-events:auto}.bajsas-media.show{display:grid;grid-template-rows:1fr auto}#custom-stream-wrapper:has(.bajsas-media.show) #custom-stream-ui{bottom:132px!important}#custom-stream-wrapper:has(.bajsas-media.show):hover #custom-stream-ui{opacity:1!important}.bajsas-media iframe{width:100%;height:100%;border:0;pointer-events:auto}.bajsas-media-lock{position:absolute;top:10px;left:50%;transform:translateX(-50%);z-index:3;padding:4px 9px;border-radius:999px;background:rgba(14,14,18,.72);color:#c9b7ef;font:10px sans-serif;pointer-events:none;backdrop-filter:blur(5px)}.bajsas-media-bar{display:flex;gap:6px;align-items:center;padding:8px;background:rgba(20,20,24,.96);color:#fff}.bajsas-media-bar input{flex:1;min-width:100px;background:#18181b;color:#fff;border:1px solid #555;padding:6px}.bajsas-media-queue{max-width:35%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;color:#adadb8;cursor:help}.bajsas-media-queue-list{display:none;position:absolute;right:10px;bottom:54px;z-index:5;width:min(330px,80vw);max-height:300px;overflow:auto;padding:7px;background:rgba(14,14,18,.97);border:1px solid rgba(255,255,255,.17);border-radius:7px;box-shadow:0 15px 40px rgba(0,0,0,.65);color:#fff}.bajsas-media-queue-list.show{display:block}.bajsas-media-queue-item{display:grid;grid-template-columns:26px 1fr;gap:7px;padding:7px;border-top:1px solid rgba(255,255,255,.07);font:11px sans-serif}.bajsas-media-queue-item:first-child{border-top:0}.bajsas-media-queue-index{display:grid;place-items:center;width:23px;height:23px;border-radius:50%;background:rgba(145,71,255,.25);color:#d8c7ff;font-weight:800}.bajsas-media-queue-title{font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bajsas-media-queue-meta{color:#adadb8;font-size:10px;margin-top:2px}
 ...
 const mediaFrame=mediaPanel.querySelector('iframe'),mediaInput=mediaPanel.querySelector('input'),mediaStatus=mediaPanel.querySelector('[data-media-status]');const mediaQueueList=document.createElement('div');mediaQueueList.className='bajsas-media-queue-list';mediaPanel.appendChild(mediaQueueList);let mediaCurrentId='',mediaRevision=0,mediaSuppressUntil=0,mediaClockOffset=0,mediaLastState=null,mediaHoverCard=null;
 const renderMediaQueue=state=>{mediaQueueList.replaceChildren();const queue=state?.queue||[];if(!queue.length){const empty=document.createElement('div');empty.className='bajsas-presence-empty';empty.textContent='Queue is empty';mediaQueueList.appendChild(empty);return}queue.forEach((item,index)=>{const row=document.createElement('div');row.className='bajsas-media-queue-item';const number=document.createElement('span');number.className='bajsas-media-queue-index';number.textContent=String(index+1);const info=document.createElement('div');const title=document.createElement('div');title.className='bajsas-media-queue-title';title.textContent=item.title+(item.year?' ('+item.year+')':'');const meta=document.createElement('div');meta.className='bajsas-media-queue-meta';meta.textContent='Requested by '+item.requestedBy;info.append(title,meta);row.append(number,info);mediaQueueList.appendChild(row)})};
 mediaStatus.addEventListener('mouseenter',()=>{renderMediaQueue(mediaLastState);mediaQueueList.classList.add('show')});mediaStatus.addEventListener('mouseleave',()=>setTimeout(()=>{if(!mediaQueueList.matches(':hover'))mediaQueueList.classList.remove('show')},100));mediaQueueList.addEventListener('mouseleave',()=>mediaQueueList.classList.remove('show'));
 ...
 const renderMedia=state=>{mediaLastState=state;if(mediaQueueList.classList.contains('show'))renderMediaQueue(state);if(Number.isFinite(Number(state.serverNow))&&Math.abs((Number(state.serverNow)-Date.now())-mediaClockOffset)>2000)mediaClockOffset=Number(state.serverNow)-Date.now();const cur=state.current;if(cur&&cur.id!==mediaCurrentId){mediaCurrentId=cur.id;const pb=cur.playback||{playing:true,position:0,updatedAt:cur.startedAt};const start=Math.max(0,Math.floor(pb.position+(pb.playing?(mediaNow()-pb.updatedAt)/1000:0)));mediaFrame.src=`${WORKER_BASE_URL}/media-player?type=${encodeURIComponent(cur.type)}&id=${encodeURIComponent(cur.providerId)}&season=${Number(cur.season||1)}&episode=${Number(cur.episode||1)}&start=${start}`;mediaFrame.onload=()=>setTimeout(()=>applyMediaPlayback(cur),700)}else if(cur&&(cur.playback?.revision||0)>mediaRevision)applyMediaPlayback(cur);if(!cur){mediaCurrentId='';mediaRevision=0;mediaFrame.src=''}mediaStatus.textContent=cur?`${cur.title}${cur.year?' ('+cur.year+')':''} • by ${cur.requestedBy} • ${state.votes}/${state.required} votes • ${state.queue.length} queued`:'Queue empty'};
 const destroyMediaPlayer=()=>{mediaPanel.classList.remove('show');mediaQueueList.classList.remove('show');mediaFrame.onload=null;mediaFrame.src='about:blank';mediaCurrentId='';mediaRevision=0;mediaSuppressUntil=0;dbg('media:player-destroyed')};
